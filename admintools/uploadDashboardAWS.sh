...
docker exec submit002 /varidata/research/software/slurmPretty/slurmnodez -f | sed -r "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2};?)?)?[mGK]//g" > /tmp/slurmnodez.txt
docker exec submit002 /varidata/research/software/slurmPretty/zqueuefull -f | sed -r "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2};?)?)?[mGK]//g" > /tmp/zqueue.txt
