...
docker exec submit002 /varidata/research/software/slurmPretty/dashboard.slurmnodez -f | sed -r "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2};?)?)?[mGK]//g" > /tmp/slurmnodez.txt
docker exec submit002 /varidata/research/software/slurmPretty/zqueuefull -f | sed -r "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2};?)?)?[mGK]//g" | sed -r "s/quick/short/g" > /tmp/zqueue.txt
